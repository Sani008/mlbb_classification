...
assist = st.slider("Jumlah Kill",0,20)
death = st.slider("Jumlah Kill",0,20)
turret = st.slider("Jumlah Kill",0,20)

if st.button("Predict"):
	data_baru = pd.DataFrame([[kill.assist,death,turret]],columns=["kill","assist","death","turret"])
	st.succes(f"Hasil Prediksi : {model.predict(data_baru)[0]}")
	st.baloons()
